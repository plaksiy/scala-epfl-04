...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'occurrencesOfLang' should work for (specific) RDD with one element AND TWO WORDS") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta Java")))
    val res = occurrencesOfLang("Java", rdd) == 1
    assert(res, "occurrencesOfLang given (specific) RDD with one element AND TWO WORDS should equal to 1")
  }
