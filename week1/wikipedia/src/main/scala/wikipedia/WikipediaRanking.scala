...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)(
      (acc: Int, art: WikipediaArticle) => if (art.text.split(" ").contains(lang)) acc + 1 else acc,
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap( art => langs intersect art.text.split(" ") map ((_, art)) )
      .groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index
      .mapValues(arts => arts.map(a => 1).sum)
      .collect()
      .toList
      .sortBy(_._2)(Ordering[Int].reverse)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd
      .flatMap( art => langs intersect art.text.split(" ") map ((_, 1)) )
      .reduceByKey(_ + _)
      .collect()
      .toList
      .sortBy(_._2)(Ordering[Int].reverse)
