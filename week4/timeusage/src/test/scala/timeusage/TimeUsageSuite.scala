...
class TimeUsageSuite extends FunSuite with BeforeAndAfterAll {
  ...
  val fields: List[String] = "tucaseid,gemetsta,gtmetsta,peeduca,pehspnon,ptdtrace"
    .split(",").toList

  val fieldsAll: List[String] =
  ...
  val attrs: List[String] = "\"20030100013280\",1,-1,44,2,2"
    .split(",").toList

  test("dbSchema can be created") {
    val schema = dfSchema(fields)

    assert(schema.fields.length == 6)
    assert(schema.fields(0).equals(StructField("tucaseid", StringType, nullable = false)))
    assert(schema.fields(1).equals(StructField("gemetsta", DoubleType, nullable = false)))
    assert(schema.fields(5).equals(StructField("ptdtrace", DoubleType, nullable = false)))
  }

  test("row() should return Row") {
    val rowObj = row(attrs)

    assert(rowObj.getString(0) == "\"20030100013280\"")
    assert(rowObj.getDouble(3) == 44)
    assert(rowObj.getDouble(5) == 2)
  }

  test("classifiedColumns") {
    val (primaryNeedsColumns, workColumns, otherColumns) = classifiedColumns(fieldsAll)

    assert(primaryNeedsColumns.nonEmpty)
    assert(workColumns.nonEmpty)
    assert(otherColumns.nonEmpty)

    assert(primaryNeedsColumns.intersect(workColumns).isEmpty)
    assert(primaryNeedsColumns.intersect(otherColumns).isEmpty)
    assert(otherColumns.intersect(workColumns).isEmpty)
  }
  ...
  test("full test") {
    ...
    val summedDs = timeUsageSummaryTyped(summaryDf)
    val finalDs = timeUsageGroupedTyped(summedDs)
